...
│  ├─ ProviderAPI.ts                (API client for /api/providers/*)
...
│  ├─ JobAPI.ts                     (API client for /api/jobs)
│  ├─ JobStatusPanel.ts             (Tree view for job history)
...
- [ ] Mobile-responsive (for Cursor/Windsurf mobile browsers)
...
- ❌ Mobile VS Code (Cursor/Windsurf): MVP not optimized
...
- Provider reputation tracking and reviews
...
- Reuse existing vscode API
- Reuse existing API client pattern
- Reuse existing styling/theming
...
- If moving to webview-based full dashboard, reuse JobAPI client logic
- Consider moving modal to sidebar webview for consistency

**For Backend Architect (Ongoing):**
- Job estimation API response needs: model_cost_per_token, provider_uptime
- Job status endpoint needs: eta_seconds, progress_percent for better UX

**For DevOps (Deployment):**
- Extension bundle adds 35 KiB to installation
- No server-side changes needed for Phase 2
- Monitor provider API latency (polling every 3s)

---

## Rollback Plan

If Phase 2 has critical issues post-deployment:
1. Disable provider status polling (turn off tree view refresh)
2. Remove job submission command from palette
3. Revert to extension.js from Phase 1 bundle
4. Continue with Phase 1 features only

---

**Document Version:** 1.0
**Last Updated:** 2026-03-24 00:15 UTC
