...
describe('Rate limiting — provider health route (max: 30/min)', () => {
  it('returns 429 on the 31st unauthenticated request to /api/providers/:id/health', async () => {
    const app = express();
    app.use(express.json());
    ...
    const statuses = await hitNTimes(app, 'get', '/api/providers/1/health', 31);
