...
describe('Rate limiting — provider activation control path (max: 30/min)', () => {
  it('blocks 31st provider activation attempt from same IP (429)', async () => {
    const app = express();
    app.use(express.json());
    ...
    const statuses = [];
    for (let i = 0; i < 31; i++) {
      const res = await request(app).post('/api/providers/provider-123/activate').send({});
      statuses.push(res.status);
    }
