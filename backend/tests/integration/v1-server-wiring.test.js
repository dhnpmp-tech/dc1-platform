...
describe('server /v1 wiring', () => {
  ...
  test('POST /v1/chat/completions forwards tools and tool_choice to provider', async () => {
    ...
    const { port } = providerServer.address();
    db.run(
      'UPDATE providers SET vllm_endpoint_url = ?, updated_at = datetime(\'now\') WHERE email = ?',
      `http://127.0.0.1:${port}`,
      'provider-wiring@test.com'
    );
    ...
    const toolsPayload = [{
      ...
      function: {
        name: 'lookup_rate',
        description: 'Lookup pricing rate',
    ...
    expect(capturedBody.tools).toEqual(toolsPayload);
