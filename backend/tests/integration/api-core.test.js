...
describe('Provider API — POST /api/providers/register', () => {
  ...
  it('normalizes os to lowercase and returns canonical setup installer_url', async () => {
    ...
    expect(res.body.installer_url).toContain('os=linux');

    const row = db.get('SELECT os FROM providers WHERE api_key = ?', res.body.api_key);
