...
/**
 * DCP-779: Provider liveness monitor + live job status endpoint tests.
 *
 * Covers:
...
describe('DCP-779: Heartbeat job_tokens update', () => {
  test('heartbeat with job_tokens updates serve_sessions total_tokens', async () => {
    ...
    // Need a real job_id for the FK constraint on serve_sessions
...
describe('DCP-779: GET /api/jobs/:id/status endpoint', () => {
  ...
  test('tokens_generated reflects serve_session total_tokens', async () => {
    ...
    ).run('sess-' + job.job_id, job.job_id, providerId);
...
describe('DCP-779: runLivenessSweep', () => {
