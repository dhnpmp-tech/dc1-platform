...
// flatParams is hoisted within the jest.mock factory (function declaration after return)
jest.mock('../db', () => {
  return {
...
function insertRenter(db, { apiKey = 'test-renter-key', balanceHalala = 100000 } = {}) {
  ...
  ).run(`renter-${Date.now()}-${Math.random().toString(36).slice(2)}@test.com`, apiKey, balanceHalala);
}
...
describe('POST /api/templates/:id/deploy', () => {
  ...
  it('returns 402 when renter balance is insufficient', async () => {
    // llama3-8b is llm-inference @ 9 halala/min x 60 min = 540 halala minimum
  ...
  it('respects pricing_class=priority with +20% surcharge', async () => {
    ...
    insertRenter(global.__testDb, { apiKey: RENTER_KEY, balanceHalala: 100000 });
    insertRenter(global.__testDb, { apiKey: RENTER_KEY_2, balanceHalala: 100000 });
    // Two providers so first deploy does not block second
    insertProvider(global.__testDb, { vramGb: 24 });
    insertProvider(global.__testDb, { vramGb: 24 });
