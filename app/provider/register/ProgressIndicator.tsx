...
export default function ProgressIndicator({ currentStep = 1 }: ProgressIndicatorProps) {
  return (
    <div
      className="mb-8 rounded-2xl border border-dc1-border bg-dc1-surface-l1 p-5 sm:p-6"
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        {STEPS.map((step) => {
          ...
          return (
            <div
              key={step.id}
              className={`rounded-xl border p-4 transition-colors ${cardClass}`}
              data-testid={`registration-progress-step-${step.id}`}
              data-state={state}
            >
