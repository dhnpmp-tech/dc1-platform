...
function isArabicModel(m: ModelListItem): boolean {
  const id = m.model_id?.toLowerCase() ?? ''
  return id.includes('allam') || id.includes('jais') || id.includes('arabic') ||
    (m.family?.toLowerCase() ?? '').includes('arabic') ||
    (m.use_cases ?? []).some(u => u.toLowerCase().includes('arabic'))
}
function getTaskType(m: ModelListItem): string {
  const id = m.model_id?.toLowerCase() ?? ''
  const uses = (m.use_cases ?? []).map(u => u.toLowerCase())
...
function SkeletonCard() {
  return (
    <div className="bg-dc1-surface-l2 border border-dc1-border rounded-xl p-5 flex flex-col gap-3 animate-pulse">
      ...
      <div className="flex gap-2"><div className="h-5 bg-dc1-surface-l3 rounded-full w-16" /><div className="h-5 bg-dc1-surface-l3 rounded-full w-12" /></div>
...
function ModelCard({ model }: { model: ModelListItem }) {
  ...
  return (
    <article className="bg-dc1-surface-l2 border border-dc1-border rounded-xl p-5 flex flex-col gap-3 hover:border-dc1-amber/30 transition-all duration-200 group">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-bold text-dc1-text-primary group-hover:text-dc1-amber transition-colors leading-tight truncate">{model.display_name}</h3>
          <p className="text-xs text-dc1-text-muted font-mono mt-0.5 truncate">{model.model_id}</p>
        </div>
        {arabic && <span className="shrink-0 text-[10px] px-2 py-0.5 rounded-full border font-medium bg-dc1-amber/10 text-dc1-amber border-dc1-amber/20">🌙 Arabic</span>}
      </div>
      <div className="flex flex-wrap gap-1.5">
        {tierBadge && <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${tierBadge.cls}`}>{tierBadge.label}</span>}
        {prewarmBadge && <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${prewarmBadge.cls}`}>{prewarmBadge.label}</span>}
        <span className="text-[10px] px-2 py-0.5 rounded-full bg-dc1-surface-l3 text-dc1-text-muted border border-dc1-border capitalize">{taskType}</span>
        {model.quantization && <span className="text-[10px] px-2 py-0.5 rounded-full bg-dc1-surface-l3 text-dc1-text-muted border border-dc1-border font-mono">{model.quantization}</span>}
      </div>
      {(model.use_cases ?? []).length > 0 && (
        <div className="flex flex-wrap gap-1">
          {model.use_cases!.slice(0, 3).map((u, i) => (
            <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-dc1-amber/5 text-dc1-amber border border-dc1-amber/15">{u}</span>
          ))}
        </div>
      )}
      <div className="bg-dc1-surface-l1 rounded-lg px-3 py-2.5 grid grid-cols-2 gap-2 text-xs">
        {vram && <div><p className="text-dc1-text-muted uppercase tracking-wide text-[9px]">VRAM</p><p className="font-semibold text-dc1-text-primary">{vram} GB</p></div>}
        {model.context_window && <div><p className="text-dc1-text-muted uppercase tracking-wide text-[9px]">Context</p><p className="font-semibold text-dc1-text-primary">{(model.context_window / 1000).toFixed(0)}K tokens</p></div>}
        {model.providers_online !== undefined && (
          <div>
            <p className="text-dc1-text-muted uppercase tracking-wide text-[9px]">Providers</p>
            <p className={`font-semibold ${(model.providers_online ?? 0) > 0 ? 'text-status-success' : 'text-dc1-text-muted'}`}>{model.providers_online ?? 0} online</p>
          </div>
        )}
        {priceHr !== null && <div><p className="text-dc1-text-muted uppercase tracking-wide text-[9px]">DCP Price</p><p className="font-extrabold text-dc1-amber">{priceHr} <span className="text-[9px] font-normal text-dc1-text-muted">SAR/hr</span></p></div>}
      </div>
      {arabic && (
      ...
      <Link href={deployHref} className="btn btn-primary w-full text-center text-sm mt-auto min-h-[44px] flex items-center justify-center">
        Deploy Model
      </Link>
    </article>
  )
...
export default function RenterModelsPage() {
  ...
  useEffect(() => {
    ...
    if (!key) { router.push('/login?role=renter&reason=missing_credentials'); return }
  ...
  const filtered = useMemo(() => models.filter(m => {
    ...
    if (filterVram !== '') {
      ...
      if (!isNaN(minV) && (m.min_gpu_vram_gb ?? m.vram_gb ?? 0) < minV) return false
    }
    if (search.trim()) {
      const q = search.toLowerCase()
      if (!`${m.model_id} ${m.display_name} ${m.family ?? ''} ${(m.use_cases ?? []).join(' ')}`.toLowerCase().includes(q)) return false
    }
    return true
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-dc1-text-secondary mt-1">Deploy Arabic-capable LLMs, embeddings, and rerankers on Saudi GPUs.</p>
          </div>
          <Link href="/renter/marketplace" className="btn btn-secondary btn-sm self-start sm:self-auto">← Back to Marketplace</Link>
        </div>
        ...
        {!loading && !error && arabicCount > 0 && (
          <div className="bg-dc1-amber/5 border border-dc1-amber/30 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-dc1-text-secondary">Bundle BGE-M3 + BGE reranker + ALLaM/JAIS — complete PDPL-compliant Arabic RAG stack.</p>
            </div>
            <Link href="/marketplace/templates?category=embedding" className="btn btn-primary shrink-0 text-sm">View Arabic RAG Templates</Link>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-3 items-center p-4 bg-dc1-surface-l1 rounded-xl border border-dc1-border">
          <div className="relative flex-1 min-w-48">
            ...
            <input type="text" placeholder="Search models…" value={search} onChange={e => setSearch(e.target.value)} className="input ps-9 w-full text-sm" />
          </div>
          <select value={filterTask} onChange={e => setFilterTask(e.target.value as TaskFilter)} className="input text-sm w-auto min-h-[44px]">
          ...
          <select value={filterTier} onChange={e => setFilterTier(e.target.value as 'all' | 'tier_a' | 'tier_b')} className="input text-sm w-auto min-h-[44px]">
            <option value="all">All Tiers</option>
            <option value="tier_a">⭐ Tier A</option>
            <option value="tier_b">✦ Tier B</option>
          </select>
          <input type="number" min="0" step="8" placeholder="Min VRAM (GB)" value={filterVram} onChange={e => setFilterVram(e.target.value)} className="input text-sm w-36 min-h-[44px]" />
          <label className="flex items-center gap-2 text-sm text-dc1-text-secondary cursor-pointer select-none min-h-[44px]">
            <input type="checkbox" checked={filterArabic} onChange={e => setFilterArabic(e.target.checked)} className="rounded" />
            🌙 Arabic only
          </label>
          <span className="text-xs text-dc1-text-muted whitespace-nowrap ms-auto">{loading ? 'Loading…' : `${filtered.length} of ${models.length} models`}</span>
        </div>

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">{[...Array(8)].map((_, i) => <SkeletonCard key={i} />)}</div>
        ) : error ? (
          ...
          <div className="text-center py-20">
            ...
            <button onClick={() => { setSearch(''); setFilterArabic(false); setFilterTask('all'); setFilterTier('all'); setFilterVram('') }} className="btn btn-outline btn-sm mt-3">Clear filters</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">{filtered.map(m => <ModelCard key={m.model_id} model={m} />)}</div>
        )}
      </div>
    </DashboardLayout>
  )
