...
type TaskType = 'chat' | 'embedding' | 'reranking' | 'image'

function getTaskType(model: ModelListItem): TaskType {
  const id = model.model_id?.toLowerCase() ?? ''
...
function getTierBadge(t: (key: string) => string, tier?: string | null) {
  if (tier === 'tier_a') return { label: `⭐ ${t('marketplace.models.tier_a')}`, cls: 'bg-dc1-amber/10 text-dc1-amber border-dc1-amber/30' }
  if (tier === 'tier_b') return { label: `✦ ${t('marketplace.models.tier_b')}`, cls: 'bg-status-info/10 text-status-info border-status-info/30' }
  if (tier === 'tier_c') return { label: t('marketplace.models.tier_c'), cls: 'bg-dc1-surface-l3 text-dc1-text-muted border-dc1-border' }
  return null
}

function getPrewarmBadge(t: (key: string) => string, prewarm?: string | null) {
  if (prewarm === 'hot') return { label: `🔥 ${t('marketplace.models.prewarm_hot')}`, cls: 'bg-status-error/10 text-status-error border-status-error/20' }
  if (prewarm === 'warm') return { label: `♨ ${t('marketplace.models.prewarm_warm')}`, cls: 'bg-dc1-amber/10 text-dc1-amber border-dc1-amber/20' }
  return null
}
...
function ModelCard({ model }: { model: ModelListItem }) {
  ...
  const tierBadge = getTierBadge(t, model.tier)
  const prewarmBadge = getPrewarmBadge(t, model.prewarm_class)
  const taskType = getTaskType(model)
  const taskLabelMap: Record<TaskFilter, string> = {
    all: t('marketplace.models.filter_all_tasks'),
    chat: t('marketplace.models.task_chat'),
    embedding: t('marketplace.models.task_embedding'),
    reranking: t('marketplace.models.task_reranking'),
    image: t('marketplace.models.task_image'),
  }
  ...
  return (
    <article className="bg-dc1-surface-l2 border border-dc1-border rounded-xl p-5 flex flex-col gap-3 hover:border-dc1-amber/30 hover:shadow-amber transition-all duration-200 group">
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {arabic && (
          <span className="shrink-0 text-[10px] px-2 py-0.5 rounded-full border font-medium bg-dc1-amber/10 text-dc1-amber border-dc1-amber/20">
            🌙 {t('marketplace.models.arabic_badge')}
          </span>
        )}
      </div>
      ...
      <div className="bg-dc1-surface-l1 rounded-lg px-3 py-2.5 grid grid-cols-2 gap-2 text-xs">
        {vram && (
          <div>
            <p className="text-dc1-text-muted uppercase tracking-wide text-[9px]">{t('marketplace.models.vram_label')}</p>
            <p className="font-semibold text-dc1-text-primary">{vram} GB</p>
          </div>
        )}
        {model.context_window && (
          <div>
            <p className="text-dc1-text-muted uppercase tracking-wide text-[9px]">{t('marketplace.models.context_label')}</p>
            <p className="font-semibold text-dc1-text-primary">{(model.context_window / 1000).toFixed(0)}K tokens</p>
          </div>
        )}
        {priceHr !== null && (
          <div className="col-span-2">
            <p className="text-dc1-text-muted uppercase tracking-wide text-[9px]">{t('marketplace.models.dcp_price_label')}</p>
            <p className="font-extrabold text-dc1-amber">{priceHr} <span className="text-[9px] font-normal text-dc1-text-muted">{t('marketplace.models.sar_hr')}</span></p>
          </div>
        )}
      </div>
      ...
      {arabic && (
        <div className="bg-status-success/5 border border-status-success/20 rounded-lg px-3 py-2 text-xs">
          <span className="text-status-success font-semibold">{t('marketplace.models.save_up_to')}</span>
          <span className="text-dc1-text-muted ml-1">{t('marketplace.models.vs_aws')}</span>
        </div>
      )}

      {/* CTA */}
      <Link href={deployHref} className="btn btn-primary w-full text-center text-sm mt-auto">
        {t('marketplace.models.deploy_model')}
      </Link>
    </article>
  )
}
    ...
    <section className="border-b border-dc1-border bg-dc1-surface-l1/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
        <p className="text-xs font-semibold text-dc1-text-muted uppercase tracking-wider mb-3">
          {t('marketplace.models.comparison_title')}
        </p>
            ...
            <div key={row.gpu} className="bg-dc1-surface-l2 rounded-xl border border-dc1-border p-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-dc1-amber">{t('marketplace.models.dcp_saudi')}</span>
                  <span className="font-extrabold text-dc1-amber">{row.dcp_sar_hr.toFixed(2)} {t('marketplace.models.sar')}</span>
                </div>
                <div className="flex items-center justify-between text-xs text-dc1-text-muted">
                  <span>Vast.ai</span>
                  <span className="line-through">{row.vast_sar_hr.toFixed(2)} {t('marketplace.models.sar')}</span>
                </div>
                <div className="mt-2 pt-2 border-t border-dc1-border flex items-center justify-between">
                  <span className="text-xs text-dc1-text-muted">{t('marketplace.models.your_savings')}</span>
      ...
      <main className="flex-1">
        ...
        <section className="border-b border-dc1-border bg-gradient-to-b from-dc1-amber/5 to-transparent">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="flex items-center gap-2 mb-3">
              <Link href="/marketplace" className="text-sm text-dc1-text-muted hover:text-dc1-amber transition-colors">{t('marketplace.models.breadcrumb_marketplace')}</Link>
              <span className="text-dc1-text-muted">/</span>
              <span className="text-sm text-dc1-text-primary font-medium">{t('marketplace.models.breadcrumb_models')}</span>
            </div>
            <h1 className="text-3xl sm:text-4xl font-bold text-dc1-text-primary mb-3">
              {t('marketplace.models.hero_title')}
            </h1>
            <p className="text-dc1-text-secondary text-lg mb-6 max-w-2xl">
              {t('marketplace.models.hero_subtitle')}
            </p>
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center gap-2 bg-dc1-surface-l1 rounded-lg px-3 py-2 border border-dc1-border">
                ...
                <span className="text-dc1-text-secondary">{t('marketplace.models.models_available')}</span>
              </div>
              <div className="flex items-center gap-2 bg-dc1-amber/10 rounded-lg px-3 py-2 border border-dc1-amber/20">
                ...
                <span className="text-dc1-amber font-medium">{t('marketplace.models.arabic_capable')}</span>
              </div>
              <div className="flex items-center gap-2 bg-dc1-surface-l1 rounded-lg px-3 py-2 border border-dc1-border">
                <span className="text-dc1-amber font-bold">⭐ {loading ? '…' : tierACount}</span>
                <span className="text-dc1-text-secondary">{t('marketplace.models.tier_a_prewarmed')}</span>
              </div>
              <div className="flex items-center gap-2 bg-status-success/10 rounded-lg px-3 py-2 border border-status-success/20">
                <span className="text-status-success font-bold">{t('marketplace.models.save_33_51')}</span>
                <span className="text-dc1-text-secondary">{t('marketplace.models.vs_aws')}</span>
              </div>
              {liveProviderCount !== null && (
                <div className="flex items-center gap-2 bg-dc1-surface-l1 rounded-lg px-3 py-2 border border-dc1-border">
                  ...
                  <span className="text-dc1-text-secondary">{t('marketplace.models.providers_live_now')}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <section className="border-b border-dc1-border bg-dc1-surface-l1/50 sticky top-0 z-10 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-48">
              ...
              <input
                type="text"
                placeholder={t('marketplace.models.search_placeholder')}
                value={search}
            ...
            >
              <option value="all">{t('marketplace.models.filter_all_tasks')}</option>
              <option value="chat">{t('marketplace.models.task_chat')}</option>
              <option value="embedding">{t('marketplace.models.task_embedding')}</option>
              <option value="reranking">{t('marketplace.models.task_reranking')}</option>
              <option value="image">{t('marketplace.models.task_image')}</option>
            </select>
            <select
              value={filterTier}
              ...
              className="input text-sm w-auto"
            >
              <option value="all">{t('marketplace.models.filter_all_tiers')}</option>
              <option value="tier_a">⭐ {t('marketplace.models.tier_a')}</option>
              <option value="tier_b">✦ {t('marketplace.models.tier_b')}</option>
            </select>
            <input
              type="number"
              min="0"
              step="8"
              placeholder={t('marketplace.models.min_vram')}
              value={filterVram}
            ...
            <label className="flex items-center gap-2 text-sm text-dc1-text-secondary cursor-pointer select-none">
              <input
                ...
                className="rounded"
              />
              🌙 {t('marketplace.models.arabic_only')}
            </label>
            <span className="text-xs text-dc1-text-muted whitespace-nowrap ms-auto">
              {loading ? t('marketplace.models.loading') : `${filtered.length} ${t('marketplace.models.of')} ${models.length} ${t('marketplace.models.models')}`}
            </span>
          </div>
        </section>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {!loading && !error && (
            <div className="mb-8 bg-dc1-amber/5 border border-dc1-amber/30 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-dc1-text-primary mb-1">{t('marketplace.models.rag_title')}</h3>
                <p className="text-sm text-dc1-text-secondary">
                  {t('marketplace.models.rag_desc')}
                </p>
              </div>
              <Link href="/marketplace/templates?category=embedding" className="btn btn-primary shrink-0 text-sm">
                {t('marketplace.models.rag_cta')}
              </Link>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              <p className="text-dc1-text-secondary mb-2">{t('marketplace.models.error_load')}</p>
              <button onClick={() => window.location.reload()} className="btn btn-secondary btn-sm mt-2">{t('marketplace.models.retry')}</button>
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-dc1-text-secondary mb-1">{t('marketplace.models.no_match')}</p>
              <button
                onClick={() => { setSearch(''); setFilterArabic(false); setFilterTask('all'); setFilterTier('all'); setFilterVram('') }}
                className="btn btn-outline btn-sm mt-3"
              >
                {t('marketplace.models.clear_filters')}
              </button>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        {!loading && !error && (
          <section className="border-t border-dc1-border bg-dc1-surface-l1">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              <h2 className="text-xl font-bold text-dc1-text-primary mb-2">{t('marketplace.models.buyer_economics_title')}</h2>
              <p className="text-dc1-text-secondary text-sm mb-6">
                {t('marketplace.models.buyer_economics_desc')}
              </p>
              <div className="overflow-x-auto">
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-dc1-border">
                      <th className="text-left py-3 px-4 text-dc1-text-muted font-medium">{t('marketplace.models.table_gpu_tier')}</th>
                      <th className="text-right py-3 px-4 text-dc1-amber font-medium">{t('marketplace.models.table_dcp_sar_hr')}</th>
                      ...
                      <th className="text-right py-3 px-4 text-status-success font-medium">{t('marketplace.models.table_you_save')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {PRICING_COMPARISON.map(row => (
                    ...
                    <tr className="bg-dc1-amber/5">
                      <td colSpan={6} className="py-3 px-4 text-xs text-dc1-text-muted italic">
                        {t('marketplace.models.table_note')}
                      </td>
                    </tr>
                  </tbody>
                </table>
